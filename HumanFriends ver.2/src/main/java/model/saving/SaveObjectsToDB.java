    ...
    private void saveToDB(String table, int typeId, Pet pet) {
        try {
            ...
            try (Connection con = DriverManager.getConnection(url, user, password)) {
                String query = "INSERT INTO ? (name, birthday, commands, animal_kind_id) VALUES (?, ?, ?, ?);";
                PreparedStatement prepSt = con.prepareStatement(query);
                prepSt.setString(1, table);
                prepSt.setString(2, pet.getName());
                prepSt.setDate(3, Date.valueOf(pet.getBirthdayDate()));
                prepSt.setString(4, String.join(",", pet.getCommands()));
                prepSt.setInt(5, typeId);
